...
export function SortableArraySection<T extends FieldValues>({
  ...
  // Starea pentru elementele individuale
  const [openItems, setOpenItems] = useState<Record<string, boolean>>({});
  // Starea pentru întreaga secțiune (implicit închisă)
  ...
  const handleAddNewItem = () => {
    ...
    setIsSectionOpen(true); // Auto-expandăm secțiunea dacă utilizatorul adaugă un element nou
  };
      ...
      <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            title="Add new item"
            className="bg-violet-600 hover:bg-violet-700 text-white rounded-full h-10 w-10 sm:h-12 sm:w-12 shadow-lg shadow-violet-900/20 p-0 flex items-center justify-center"
          >
          ...
          <CollapsibleTrigger asChild>
            <Button 
              ...
              variant="ghost" 
              size="icon" 
              className="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-zinc-900/50 hover:bg-zinc-800 border border-zinc-800 text-zinc-400 hover:text-white"
            >
